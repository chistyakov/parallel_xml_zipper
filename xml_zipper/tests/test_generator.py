...
def test_generate_xml_with_1_objects(
    mock_generate_id, mock_generate_name, mock_generate_objects_count
):
    xml_ = g.generate_xml()
    assert xml_ == (
        "<root>"
...
def test_generate_xml_with_3_objects(
    mock_generate_id, mock_generate_name, mock_generate_objects_count
):
    mock_generate_name.side_effect = ["foo", "bar", "baz"]
    xml_ = g.generate_xml()
    assert xml_ == (
        "<root>"
        '<var name="id" value="bd2ca695-346d-467e-a061-51eb405f4954" />'
